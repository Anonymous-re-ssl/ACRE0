...
from .mydataset import ImageFolder, ImageFilelist, ImageList, SimpleDataset
...
class UDADataset:
    ...
    def get_dsets_cifar(self):
        ...
        train_data = np.load(f'{self.train_path}/data.npy')
        train_target = np.load(f'{self.train_path}/target.npy')
        train_gt = np.load(f'{self.train_path}/gt.npy')

        val_data = np.load(f'{self.train_path}/data.npy')
        val_target = np.load(f'{self.train_path}/target.npy')
        val_gt = np.load(f'{self.train_path}/gt.npy')

        test_data = np.load(f'{self.test_path}/data.npy')
        test_target = np.load(f'{self.test_path}/target.npy')
        test_gt = np.load(f'{self.test_path}/gt.npy')
        ...
        test_dataset = SimpleDataset(test_dataset)


        val_dataset.dataset["gt"][val_dataset.dataset["gt"] == -1] = 10   # equals args.n_class
        test_dataset.dataset["gt"][test_dataset.dataset["gt"] == -1] = 10  # equals args.n_class


        self.train_dataset = DatasetWithIndicesWrapper(train_dataset.dataset["data"], train_dataset.dataset["target"],
                                                       self.train_transforms, self.test_transforms)
        self.val_dataset = DatasetWithIndicesWrapper(val_dataset.dataset["data"], val_dataset.dataset["gt"],
                                                     self.test_transforms, self.test_transforms)
        self.test_dataset = DatasetWithIndicesWrapper(test_dataset.dataset["data"], test_dataset.dataset["gt"],
                                                      self.test_transforms, self.test_transforms)
    ...
    def get_loaders(self, shuffle=True, num_workers=0, class_balance_train=False):
        ...
        """
        if not self.train_dataset: self.get_dsets_cifar()
